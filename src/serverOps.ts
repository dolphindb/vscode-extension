...
function resultFormat (res: string, start: Date, end: Date): string {
    return (
        ...
        (end.valueOf() - start.valueOf()).toString() +
        'ms]\n'
    )
}
...
export async function dolphindbExecuteCode () {
    ...
    if (code.length === 0) 
        code = textEditor.document.getText(new vscode.Range(new vscode.Position(currentLine, 0), new vscode.Position(currentLine + 1, 0)))
...
export async function dolphindbShowInfo (node: vscode.TreeItem) {
    let code = node.label
    let start = new Date()
    let { data: data } = await api.executeCode(currentCfg.ip, currentCfg.port, code.toString(), context.sessionID, enableSSL)
    let end = new Date()
    let { data: env } = await api.fetchEnv(currentCfg.ip, currentCfg.port, context.sessionID, enableSSL)
    // every time after runing code, update ENV for variable explorer
    context.ENV = VariableInfo.extractEnv(env.object[0])
...
export async function dolphindbChooseServer () {
    ...
    vscode.window.showQuickPick(descs).then(desc => {
        if (desc === undefined) 
            return
...
export async function dolphindbAddServer () {
    ...
    address = _.filter(elem => elem !== null && typeof elem.name === 'string' && typeof elem.ip === 'string' && !isNaN(elem.port), address)
...
export async function dolphindbSSL () {
    ...
    vscode.window.showQuickPick(sslOptions).then(ssl => {
        if (ssl === undefined) 
            return
