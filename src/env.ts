...
import type { IDolphindbObject, IDolphindbAttr } from './api'

import { DolphindbContext } from './context'
import * as path from 'path'

const DATA_FORM = ['scalar', 'pair', 'vector', 'set', 'dictionary', 'matrix', 'table']
...
export class DolphindbEnvProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
    ...
    constructor (private context: DolphindbContext) {
        const env = new Map()
        for (let i = 0; i < DATA_FORM.length; i++) env.set(DATA_FORM[i], [])
    ...
    getChildren (element?: vscode.TreeItem): vscode.ProviderResult<vscode.TreeItem[]> {
        if (element && element.contextValue === 'variableInfo')
            // empty
            return []
        else if (element && element.contextValue === 'variableType') return this.getVariableInfo(element)
        else return this.variableTypes
    }
    
    private getVariableInfo (parent: vscode.TreeItem): VariableInfo[] {
        return this.context.ENV.get(parent.label.toString())
    }
}

/** VariableItem represent a variable type */
export class VariableType extends vscode.TreeItem {
    override contextValue = 'variableType'
...
export class VariableInfo extends vscode.TreeItem {
    override contextValue = 'variableInfo'
    ...
    constructor (
        ...
        super(label, collapsibleState)
        this.tooltip = `row:${this.row} col:${this.col}`
        this.description = `${this.shared} <${this.dataType}> row: ${this.row} col: ${this.col} [${this.byteSize}B]`
    }
