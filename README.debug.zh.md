...
本教程旨在介绍如何使用VSCode插件调试脚本。调试功能能够实时追踪脚本执行、查看变量值和函数调用栈等信息。通过本教程，您将了解调试功能的组件和工作原理，以及如何使用该功能进行脚本调试。

## 2. 概述
首先简单介绍一下DolphinDB 的 VSCode 插件的组成部分及其功能。
...
## 5. 小结
本教程详细介绍了使用VSCode插件调试脚本的基本用法和步骤。通过设置断点、启动调试、控制执行过程和查看调试信息，您可以实现对脚本的精确调试和变量监控。这将提升您的脚本开发效率，并帮助您解决调试过程中的问题。未来，我们可能会进一步改进和扩展该功能，以满足更多用户需求。